...
void clockdata(char bits_in){                              // Esta rotina envia os dados para o PCD8544
  ...
  for (bitcnt = 8; bitcnt > 0; bitcnt--){
    PORTC&=~(1<<SCK);                                // Parar o clock
    if ((bits_in & 0x80) == 0x80) PORTC|=(1<<SDI); // O pcd8544 "clock in" o MSB primeiro
    else PORTC&=~(1<<SDI);
    PORTC|=(1<<SCK);                               // Os dados sao enviados nas transições ascendentes de clock
...
void writedata(char data_in){   // Com esta rotina, agora método, envia-se um byte para o LCD, este byte corresponde a uma coluna com um pixel de largura e 8 pixeis de altura (1x8), 0xFF coluna toda a negro
  PORTC|=(1<<DC);       // Seleccionar o registo de Dados
  PORTC&=~(1<<SCE);       // Seleccionar o chip
  clockdata(data_in);           // "Clock in" :) os bits de dados
  PORTC|=(1<<SCE);      // Libertar o chip
}
...
void writecom(char command_in){  // Esta rotina, agora um método, serve para enviar comandos para o controlador do LCD, desligar, inverter pixeis, tipo de endereçamento, etc.
  PORTC&=~(1<<DC);         // Seleccionar o registo de Comandos
  PORTC&=~(1<<SCE);        // Seleccionar o chip
...
void initlcd(void){        // Iniciar o LCD, tem de se usar isto antes de fazer qualquer coisa no LCD! Se não se defenir a tensão do LCD este pode queimar.
  PORTC|=(1<<RES); // Meter o pin reset a 1
  PORTC|=(1<<SCE); // Desactivar o chip
  PORTC&=~(1<<RES);  // Fazer reset ao LCD
  delay_ms(100);              // Conforme diz no datasheet do pcd8544, convem esperar 100ms ate o poder acordar
  PORTC|=(1<<RES); // Acordar o LCD do estado RESET
...
void put(char valor){
int i;            // Traduz os caracteres para um formato reconhecido pelo PCD8544 e envia-os para o LCD. Cada caracter é um conjunto de 5 bytes.
  for (i = 0; i < 5; i++){
    writedata(nokiabyte[valor - 0x20][i]); // Vai buscar o byte ao array de caracteres e envia-o para o LCD
  }
...
void putstr(const char *s){  // Colocar uma frase/conjunto de caracteres (String) no LCD.
  while(*s){put(*s++);}  // Apontar para um caracter no formato ASCII e escrever um caracter de cada vez
}
